...
import { getUploadUrl, saveProviderFormCheckVideo } from "@/lib/actions/videos";

type RecordState = "idle" | "uploading" | "done" | "error";

interface ProviderFormRecordProps {
  exerciseId: string;
  patientId: string;
  onUploadingChange?: (uploading: boolean) => void;
}

export function ProviderFormRecord({
  exerciseId,
  patientId,
  onUploadingChange,
}: ProviderFormRecordProps): React.JSX.Element {
  ...
  async function handleRecordingComplete(blob: Blob, _duration: number): Promise<void> {
    ...
    const metaResult = await saveProviderFormCheckVideo(patientId, exerciseId, storagePath);
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 text-sm text-muted font-medium mt-2"
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          {recordState === "done" && (
            <div className="flex flex-col items-center gap-4 py-6 text-center">
              ...
              <p className="text-sm text-muted">Your patient will be able to see this video.</p>
